...
def get_feed_eqn():
    ...
    motor_dynemics=np.array([[Jm[0]/r[0]*q1_dot_dot + (Bm[0]+kb[0]*km[0]/R[0])/r[0]*q1_dot],
                             [Jm[1]/r[1]*q2_dot_dot + (Bm[1]+kb[1]*km[1]/R[1])/r[1]*q2_dot],
                             [Jm[2]/r[2]*q3_dot_dot + (Bm[2]+kb[2]*km[2]/R[2])/r[2]*q3_dot]])
    final=motor_dynemics + D@q_dot_dot
    for i in range(len(final)):
        final[i]=final[i]*R[i]/km[i]
    eqn=sym.Array(final)
    return sym.simplify(eqn)
...
def func(t,y):
    ...
    # disterbance=np.array([[np.random.normal(scale=0.1)*R[0]/km[0]],
    #                      [np.random.normal(scale=0.1)*R[1]/km[1]],
    #                      [np.random.normal(scale=0.1)*R[2]/km[2]]])
    # temp=temp+disterbance
    ...
    d1=temp[0][0].as_coefficients_dict()[1]+disturbance
...
D=sym.simplify(D_calculator(l2))
eqn1=dynamical_equation(D)
eqn=sym.simplify(eqn1)
print(eqn)

D_error=sym.simplify(D_calculator(l2*1.2))
eqn1=dynamical_equation(D_error)
eqn_error=sym.simplify(eqn1)
...
while(ode_eqn.t<6):
    ...
    V1 = q1_dot_dot_d + kp*(q1_d-q1) + kd*(q1_dot_d-q1_dot)
    V2 = q2_dot_dot_d + kp*(q2_d-q2) + kd*(q2_dot_d-q2_dot)
    V3 = d3_dot_dot_d + kp*(d3_d-d3) + kd*(d3_dot_d-d3_dot)

    v = eqn_error.subs([('q1_dot_dot',V1),('q2_dot_dot',V2),('q3_dot_dot',V3), ('q1_dot',q1_dot),('q2_dot',q2_dot),('q3_dot',d3_dot), ('q1',q1),('q2',q2),('q3',d3)])
    
    v1 = v[0][0]
    v2 = v[1][0]
    v3 = v[2][0]

    disturbance=np.random.normal(scale=0.1)*R[0]/km[0]
...
with open('data\\5d1.pkl', 'wb') as file:
